...
class pgs {
   ...
   */
  public function adicionar($item=null) {

    if ('array' !== gettype($item)){
      $this->_error['adicionar'] = 'Item precisa ser um array.';
      throw new Argument($this->_error['adicionar']);
    }
    ...
    foreach($tipos as $elm=>$valor){
      ...
      if(isset($item[$elm])){
        if(strpos($validos,gettype($item[$elm])) === false ||
          ...
          $this->_error['adicionar']['itens'] = "Valor invalido passado para $elm.";
          throw new Argument($this->_error['adicionar']['itens']);
        }
      }elseif($obrigatorio){
        $this->_error['adicionar']['obrigatorio'] = "O item adicionado precisa conter $elm.";
        throw new Argument($this->_error['adicionar']['obrigatorio']);
      }
    }
   ...
   * @return void
   */
  public function cliente($args=array()) {
   ...
   * @param array $args Array associativo contendo as configurações que você deseja alterar
   */
  public function mostra ($args=array()) {
    $default = array(
      'print'       => true,
      'open_form'   => true,
      'close_form'  => true,
